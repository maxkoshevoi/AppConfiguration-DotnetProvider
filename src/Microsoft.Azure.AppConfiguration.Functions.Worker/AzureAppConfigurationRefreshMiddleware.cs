...
namespace Microsoft.Azure.AppConfiguration.Functions.Worker
{
    ...
    internal class AzureAppConfigurationRefreshMiddleware : IFunctionsWorkerMiddleware
    {
        ...
        {
            // Returns false if any durable function packages are loaded, true otherwise
            return !AppDomain.CurrentDomain.GetAssemblies().Any(assembly => assembly.GetName().Name.StartsWith("Microsoft.DurableTask"));
        }
    }
}
